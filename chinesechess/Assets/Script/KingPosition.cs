...
//			blackclick.CanMove = false;
            ChessControl.NextPlayerTipStr = "红色棋子胜利";
            ChessControl.TrueOrFalse = false;
			return;
		} else if (board.chess [Shuai_y, Shuai_x] != 8) {
	//		blackclick.CanMove = false;
            ChessControl.NextPlayerTipStr = "黑色棋子胜利";
            ChessControl.TrueOrFalse = false;
			return ;
		}
		bool BOL;//bool 值
		for (int i=0; i<9; i++) {
			for(int j=0;j<10;j++){
				...
				case 2:
					...
					if(BOL)
                        ChessControl.NextPlayerTipStr = "帅被車将军了";
				
					break;
				case 3:
					BOL= rules.IsValidMove(board.chess,i,j,Shuai_x,Shuai_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "帅被马将军了";
					break;
				case 4:
					BOL= rules.IsValidMove(board.chess,i,j,Shuai_x,Shuai_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "帅被炮将军了";
					break;
			
				case 7:
					BOL= rules.IsValidMove(board.chess,i,j,Shuai_x,Shuai_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "帅被兵将军了";
					break;
				case 9:
					BOL= rules.IsValidMove(board.chess,i,j,Jiang_x,Jiang_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "将被車将军了";
					break;
				case 10:
					BOL= rules.IsValidMove(board.chess,i,j,Jiang_x,Jiang_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "将被马将军了";
					break;
				case 11:
					BOL= rules.IsValidMove(board.chess,i,j,Jiang_x,Jiang_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "将被炮将军了";
					break;
				case 14:
					BOL= rules.IsValidMove(board.chess,i,j,Jiang_x,Jiang_y);
					if(BOL)
                        ChessControl.NextPlayerTipStr = "将被兵将军了";
					break;
				}
			}
		}
